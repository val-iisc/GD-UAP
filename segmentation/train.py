    ...
    for i in range(max_iter):
        ...
        if prior_type == 'with_data':
            rander = np.random.randint(
                low=0, high=(imgs.size()[0]-train_batch_size))
            images = get_training_data(
                img_list[rander:rander+train_batch_size], img_path, size, net_name)
        ...
        if not stop_check and ((check_dif > 200 and rescaled == True) or check_dif == 400):
            ...
            cur_batch = torch.autograd.Variable(
                torch.zeros(val_batch_size, 3, 513, 513)).cuda()
            for j in range(iters):
                ...
                _, true_predictions = torch.max(
                    out_normal, 1)  # .cpu().data.numpy()
                _, ad_predictions = torch.max(
                    out_pert, 1)  # .cpu().data.numpy()
                not_flip = torch.sum(
                    torch.eq(true_predictions, ad_predictions).float()).cpu().data.numpy()
                temp += (val_batch_size*size*size - not_flip) / \
                    float(val_batch_size*size*size)
            current_rate = temp/float(iters)
            print('current_per_pixel_flipping_rate',
            ...
            if current_rate > fool_rate:
                ...
                im = v.cpu().data.numpy()
                name = os.path.join('perturbations', 'new_' +
                                    net_name+'_'+prior_type+'.npy')
                np.save(name, im)
...
def main():
    ...
    parser.add_argument('--network', default='dl_vgg16',
                        help='The network eg. Deeplab Large FOV VGG16')
    ...
    parser.add_argument(
        '--im_path', help='The location of the training images.')
    parser.add_argument('--im_list', default='None',
