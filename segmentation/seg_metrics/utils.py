...
def mat_to_png(location, file_ids, conv_dict):
    ...
    for id in file_ids:
        ...
        img = scipy.io.loadmat(os.path.join(
            location, id+'_blob_0.mat'))['data']
...
def hist_maker(predict_loc, gt_loc, file_id_list, batch_size=20, ignore_label=255, classes=21, gpu=True):
    ...
    for i in range(max_iter):
        cur_batch = file_id_list[batch_size *
                                 i:min(len(file_id_list), batch_size*(i+1))]
        predict_tensor, gt_tensor = tensor_maker(
            cur_batch, predict_loc, gt_loc, gpu=gpu)
        hist_batch = hist_per_batch(
            predict_tensor, gt_tensor, ignore_label=255, classes=21)
...
def mean_iou(hist_matrix, class_names):
    ...
    for i in range(classes):
        class_scores[i] = hist_matrix[i, i] / \
            (max(1, np.sum(hist_matrix[i, :]) +
                 np.sum(hist_matrix[:, i])-hist_matrix[i, i]))
...
def freq_weighted_miou(hist_matrix, class_names):
    ...
    for i in range(classes):
        class_scores[i] = np.sum(
            hist_matrix[i, :])*hist_matrix[i, i]/(max(1, np.sum(hist_matrix[i, :])))
