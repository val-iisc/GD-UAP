...
def train(adv_net, net, in_im, ad_im, opt_layers, net_name,train_type,batch_size,img_list_file=None):
    ...
    ### Saturation Measure
    saturation = tf.div(tf.reduce_sum(tf.to_float(tf.equal(tf.abs(ad_im),10))),tf.to_float(tf.size(ad_im)))
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(max_iter):
            ...
            if not stop_check and ((check_dif>200 and rescaled == True) or check_dif==400):
                ...
                print('current_val_fooling_rate', current_rate,'current_iter',i)
...
def main():
    ...
    parser.add_argument('--batch_size', default=25, help='The batch size to use for training and testing')
    ...
    train(adv_net, net, inp_im, ad_im, opt_layers,args.network,args.prior_type,int(args.batch_size),args.img_list)
