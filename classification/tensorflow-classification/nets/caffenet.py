...
def model(image, weights, biases, keep_prob=1.0):
    ...
    with tf.name_scope("conv1"):
        layers['conv1'] = conv_layer(
            image, weights['conv1'], biases['conv1'], s=4, padding='VALID')
    ...
    with tf.name_scope("conv2"):
        layers['conv2'] = conv_layer(
            layers['norm1'], weights['conv2'], biases['conv2'], group=2)
    ...
    with tf.name_scope("conv3"):
        layers['conv3'] = conv_layer(
            layers['norm2'], weights['conv3'], biases['conv3'])

    with tf.name_scope("conv4"):
        layers['conv4'] = conv_layer(
            layers['conv3'], weights['conv4'], biases['conv4'], group=2)

    with tf.name_scope("conv5"):
        layers['conv5'] = conv_layer(
            layers['conv4'], weights['conv5'], biases['conv5'], group=2)
...
def caffenet(input):
    ...
    for name in net.keys():
        weights[name] = tf.Variable(tf.constant(
            net[name]['weights']), dtype='float32', name=name+"_weight", trainable=False)
        biases[name] = tf.Variable(tf.constant(
            net[name]['biases']), dtype='float32', name=name+"_bias", trainable=False)
