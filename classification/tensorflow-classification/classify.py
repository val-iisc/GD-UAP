...
def validate_arguments(args):
    nets = ['vggf', 'caffenet', 'vgg16', 'vgg19',
            'googlenet', 'resnet50', 'resnet152', 'inceptionv3']
...
def predict(net, im_path, in_im, net_name):
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in inds:
            print '{:.4f}\t{:}'.format(
                softmax_scores[0, i], synset[i].strip().split(',')[0])
...
def main():
    ...
    parser.add_argument(
        '--img_path', default='misc/sample.jpg',  help='Path to input image')
    parser.add_argument('--evaluate', default=False,
                        help='Flag to evaluate over full validation set')
    parser.add_argument(
        '--img_list',  help='Path to the validation image list')
    parser.add_argument(
        '--gt_labels', help='Path to the ground truth validation labels')
    parser.add_argument('--batch_size', default=50,
                        help='Batch size for evaluation code')
