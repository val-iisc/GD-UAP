...
def train(params):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        if args.checkpoint_path != '':
            print(args.checkpoint_path.split(".")[0])
...
def test(params,adv_image):
    ...
    #left  = dataloader.left_image_batch
    #right = dataloader.right_image_batch
    #pert_load = np.random.choice([10.0/256.0,-10.0/256.0],size=(1,256,512,3))#np.load(adv_image)
    #pert_load = np.random.uniform(high=100/256.0, low = 100/256.0, size = (1,256,512,3))
    #pert_load = np.zeros((1,256,512,3))
    ...
    print(params)
    model = MonodepthModel(params, args.mode, None, None,input_final)
    net_varlist = {v.name: v for v in tf.get_collection(tf.GraphKeys.VARIABLES) if v.name not in ['input_image','noise_image:0']}
    ...
    input_np[:,:,:,0] = 0.52760133
    input_np[:,:,:,1] = 0.52509456
    input_np[:,:,:,2] = 0.52378568
