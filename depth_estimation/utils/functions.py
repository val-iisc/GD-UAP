...
def rand_crop(img_temp,dim=[256,512]):
    h =img_temp.shape[0]
    w = img_temp.shape[1]
    trig_h=trig_w=False
    if(h>dim[0]):
        h_p = int(random.uniform(0,1)*(h-dim[0]))
        img_temp = img_temp[h_p:h_p+dim[0],:,:]
    elif(h<dim[0]):
        trig_h = True
    if(w>dim[1]):
        w_p = int(random.uniform(0,1)*(w-dim[1]))
        img_temp = img_temp[:,w_p:w_p+dim[1],:]
    elif(w<dim[1]):
        trig_w = True
    if(trig_h or trig_w):
        pad = np.zeros((dim[0],dim[1],3),dtype='uint8')
        pad[:,:,0] += 104
        pad[:,:,1] += 117
        pad[:,:,2] += 123
...
def randomizer(img_temp):
    ...
    if(scale_p>.5):
        ...
        scale = 1
    #print('scale',scale)
    if(img_temp.shape[0]<img_temp.shape[1]/2.0):
        ratio = dim[0]*scale/float(img_temp.shape[0])
    else:
        ratio = dim[1]*scale/float(img_temp.shape[1])
    ...
    #print('blur',blur_p,blur_val)
    img_temp = rand_crop(img_temp)
    #print('crop',img_temp.shape)
    #img_temp = img_temp.astype('float')
    return img_temp
                               
def img_preprocess_random(img_path, size=224):
...
def img_preprocess_depth(img_path, size=224,augment=False):
    #mean = [128, 128, 128]
    img = imread(img_path)
    #print(img.shape)
    #print(img_path)
    ...
    if len(img.shape) == 2:
        img = np.dstack([img,img,img])
    img = resize(img,[256,512],mode='constant',preserve_range=True)/256.0

    return img
def new_img_preprocess_inception(img_path, size=224,augment=False):
    mean = [128, 128, 128]
    img = imread(img_path)
    #print(img_path)
    if img is None:
